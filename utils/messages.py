...
def get_welcome_message(full_name: str, invited_by: bool = False) -> str:
    ...
    if invited_by:
        msg += "Thank you for joining via invite link, the inviter has received 2 points.\n"
...
def get_about_message() -> str:
    ...
    return (
        ...
        "- Use redemption codes (per code rules)\n"
        f"- Join channel: {CHANNEL_URL}\n"
        "\n"
        "How to use:\n"
        "1. Start verification on website and copy the full verification link\n"
        "2. Send /verify, /verify2, /verify3, /verify4 or /verify5 with the link\n"
        "3. Wait for processing and check results\n"
        "4. Bolt.new verification auto-fetches code, for manual query use /getV4Code <verification_id>\n"
        "\n"
...
def get_help_message(is_admin: bool = False) -> str:
    ...
    msg = (
        ...
        "/use <code> - Redeem code for points\n"
        f"/verify <link> - Gemini One Pro verification (-{VERIFY_COST} pts)\n"
        f"/verify2 <link> - ChatGPT Teacher K12 verification (-{VERIFY_COST} pts)\n"
        f"/verify3 <link> - Spotify Student verification (-{VERIFY_COST} pts)\n"
        f"/verify4 <link> - Bolt.new Teacher verification (-{VERIFY_COST} pts)\n"
        f"/verify5 <link> - YouTube Student Premium verification (-{VERIFY_COST} pts)\n"
        "/getV4Code <verification_id> - Get Bolt.new verification code\n"
        "/help - View this help info\n"
    ...
    if is_admin:
        msg += (
            ...
            "/genkey <code> <points> [uses] [days] - Generate code\n"
            "/listkeys - View code list\n"
