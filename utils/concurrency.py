"""Concurrency control tools (optimized version)
...
def _calculate_max_concurrency() -> int:
    ...
    try:
        ...
        # Take minimum of both and set bounds
...
# Create separate semaphores for different verification types
# This prevents one type from blocking others
...
def get_concurrency_stats() -> Dict[str, Dict[str, int]]:
    ...
    Returns:
        dict: Concurrency info for each verification type
    """
    stats = {}
    for vtype, semaphore in _verification_semaphores.items():
        # Note: _value is internal attribute, may change in different Python versions
        try:
...
def adjust_concurrency_limits(multiplier: float = 1.0):
    ...
    new_limit = max(5, min(new_base // 3, 50))  # 5-50 per type
        ...
        while True:
            try:
                ...
                if cpu > 80 or memory > 85:
                    ...
                    # Load low, can increase concurrency
